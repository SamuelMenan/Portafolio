...
const testimonialsByLang: Record<Props["lang"], Testimonial[]> = {
  es: [
    {
      name: "Laura Torres Mejia",
      role: "Colaboradora en AlmasEnAccion",
      quote:
    ...
    {
      name: "Andres Felipe Rojas",
      role: "Compañero de equipo frontend",
      quote:
...
export default function WireframeTestimonials({ mobile, lang }: Props) {
  const localizedTestimonials = testimonialsByLang[lang]
    ...
    <section id="testimonials" className={`border-b-2 border-black py-12 scroll-mt-20 ${mobile ? "py-8" : ""}`}>
      ...
      <div className={`grid gap-4 ${mobile ? "grid-cols-1" : "grid-cols-2 lg:grid-cols-4"}`}>
        {localizedTestimonials.map((testimonial) => (
          <article key={testimonial.name} className="border-2 border-black bg-gray-50">
            <header className="border-b border-black px-4 py-3 flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-xs font-bold uppercase tracking-wide leading-tight">{testimonial.name}</p>
                <p className="text-xs text-gray-600 leading-tight mt-1">{testimonial.role}</p>
              </div>
            </header>

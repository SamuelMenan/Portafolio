...
export default function WireframeTestimonios({ mobile }: Props) {
  return (
    <section className={`border-b-2 border-black py-12 ${mobile ? "py-8" : ""}`}>
      <div className="flex items-center gap-4 mb-5">
        <span className="text-xs tracking-widest uppercase font-bold">05 — Testimonios</span>
        <div className="flex-1 border-t border-black" />
      </div>

      <div className={`mb-8 ${mobile ? "" : "max-w-3xl"}`}>
        <p className="text-xs tracking-[0.25em] uppercase text-gray-500 mb-2">Referencias</p>
        <h2 className={`font-bold tracking-tight leading-tight mb-3 ${mobile ? "text-2xl" : "text-3xl"}`}>
          Socios y colaboradores que respaldan mi trabajo
        </h2>
        <p className="text-sm text-gray-700 leading-relaxed">
          Estos testimonios resumen cómo abordo cada proyecto: análisis de requisitos, planificación de
          arquitectura y ejecución con foco en calidad, mantenibilidad y resultados claros.
        </p>
      </div>

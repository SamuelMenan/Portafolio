...
export default function LanguageSwitch({ value, onChange }: Props) {
  const [loadingLang, setLoadingLang] = useState<'es' | 'en' | null>(null)

  function handleLanguageChange(nextLang: 'es' | 'en') {
    if (nextLang === value || loadingLang) return

    setLoadingLang(nextLang)
    onChange(nextLang)
    window.setTimeout(() => setLoadingLang(null), 420)
  }
  ...
  return (
    <div className="language-switch" role="group" aria-label="Selector de idioma">
      <button
        type="button"
        data-loading={loadingLang === 'es'}
        aria-busy={loadingLang === 'es'}
        className={`portfolio-action portfolio-action--chip language-switch__chip ${value === 'es' ? 'is-active' : ''}`}
        onClick={() => handleLanguageChange('es')}
        aria-pressed={value === 'es'}
        aria-label="Cambiar idioma a español"
      >
        <span className="portfolio-action__spinner language-switch__spinner" aria-hidden="true" />
      ...
      <button
        type="button"
        data-loading={loadingLang === 'en'}
        aria-busy={loadingLang === 'en'}
        className={`portfolio-action portfolio-action--chip language-switch__chip ${value === 'en' ? 'is-active' : ''}`}
